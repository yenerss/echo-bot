...
module.exports = client => {
  ...
  client.getFooter = (es, stringurl = null) => {
    ...
    if(!text || text.length < 1) text = `${client.user.username} | By: yener5855#6186`;
  ...
  client.getAuthor = (authorname = null, authoricon = null, authorurl = null) => {
    ...
    if(!name || name.length < 1) name = `${client.user.username} | By: yener5855#6186`;
    if(!iconURL || iconURL.length < 1) iconURL = `${client.user.displayAvatarURL()}`;
    if(!url || url.length < 1) url = `https://discord.com/invite/Yfb2fnkduE`;
    ...
    //verify the iconURL
    if(!url.startsWith("https://") && !url.startsWith("http://")) url = `https://discord.com/invite/Yfb2fnkduE`;
    if(!iconURL.startsWith("https://") && !iconURL.startsWith("http://")) iconURL = client.user.displayAvatarURL();
    if(![".png", ".jpg", ".wpeg", ".webm", ".gif"].some(d => iconURL.toLowerCase().endsWith(d))) iconURL = client.user.displayAvatarURL();
    //return the footerobject
  ...
  */
  ...
  client.on("guildCreate", async guild => {
    ...
    for(const owner of config.ownerIDS){
      //If the Owner is yener, and the Bot is in not a yener Development, Public Bot, then dont send information!
      if(owner == "809533218205466674"){
        let yenerGuild = client.guilds.cache.get("773668217163218944");
        if(yenerGuild && !yenerGuild.me.roles.cache.has("779021235790807050")){
          continue; 
        }
      }
