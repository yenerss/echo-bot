    ...
    try {
      if(!message.guild.me.permissions.has([Permissions.FLAGS.MANAGE_NICKNAMES]))      
        return message.reply({embeds : [new MessageEmbed()
          ...
          .setTitle(`<:no:1249336451590651924> **I am missing the Permission to Manage Nicknames of others**`)
        ]})
      ...
      if (!kickmember)
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:no:1249336451590651924> **You forgot to ping a Member**`)
      ...
      if(!args[1])
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:no:1249336451590651924> **You forgot to add a Nickname**`)
          .setDescription(`Usage: \`${prefix}nickname @User newnickname\``)
        ]});
      ...
      if(nickname.length > 32)
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:no:1249336451590651924> **The Nickname must have smaller then 32 Characters**`)
          .setDescription(`Usage: \`${prefix}nickname @User newnickname\``)
        ]});
      kickmember.setNickname(nickname).then(member => {
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.color)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:yes:1249336349761339454> **Successfully changed the username of \`${kickmember.user.tag}\` to \`${nickname}\`**`)
        ]});
      }).catch(e=>{
